      ...
      <VStack spacing={4} align="center">
        ...
        <Wrap mt={2} justify="center">
          {psychologist.perfil?.especialidades?.map((specialization, index) => (
            <Badge key={index}  backgroundColor="primary.400" variant="solid">
              {specialization}
            </Badge>
          )) || 'Sem especialidades'}
        </Wrap>
  ...
  const fetchPsychologists = async () => {
    try {
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify(adaptedFilters),
      });
    ...
    >
      {psychologists.map((psychologist) => (
        <PsychologistCard
          ...
          onClick={() => onCardClick(psychologist)}
        />
      ))}
    </Grid>
  );
