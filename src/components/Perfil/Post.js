...
const Post = () => {
  ...
  const fetchPosts = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`${BACKEND_URL}/files/${token}`, {
        method: 'GET',
        headers: {
  ...
  const handleAddPost = async () => {
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/upload/${token}`, {
        method: 'POST',
        body: formData,
      });
          ...
          {posts.map((post) => (
            <Box key={post._id} bg="white" p={4} borderRadius="lg" boxShadow="md">
              ...
              <Text color="gray.600" mt={2} mb={4}>
                {post.posts?.content || 'Sem conteúdo'}
              </Text>
              <Text fontSize="sm" color="gray.500" mb={2}>
                Curtidas: {post.posts?.likes || 0}
              </Text>
              <Flex justify="space-between" align="center" mt={4}>
                <IconButton
                  ...
                  icon={<DeleteIcon />}
                  colorScheme="red"
                  onClick={() => handleDeletePost(post.posts?.filename, post._id)}
                />
                <Button
                  size="sm"
